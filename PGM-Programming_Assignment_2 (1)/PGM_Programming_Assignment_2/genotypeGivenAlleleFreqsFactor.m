...
%the number of the genotype is defined as (numAlleles-1)*numAlleles/2+numAlleles;
%The number of genotypes is (number of alleles choose 2) + number of 
% alleles -- need to add number of alleles at the end to account for 
% homozygotes

genotypeFactor.card=(numAlleles-1)*numAlleles/2+numAlleles;

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));

assignAllels=ones(numAlleles*numAlleles,2);

for i=1:numAlleles
    for j=1:numAlleles
     genotypeFactor.val(1,allelesToGenotypes(i,j))=alleleFreqs(i)*alleleFreqs(j)+genotypeFactor.val(1,allelesToGenotypes(i,j));
    end
end
