...
% Your code here:
%assignments= IndexToAssignment(1:K*K*K, [K,K,K]);
factorValue=ones(1,K*K*K);
%assign top 2000 frequency triplet 
for i=1:2000
    factorValue(1,AssignmentToIndex(tripletList(i).chars,[K,K,K]))= tripletList(i).factorVal;
end

for i=1:n-2
    factors(i).var=[i,i+1,i+2];
    factors(i).card=[K,K,K];
    factors(i).val=factorValue;
end
