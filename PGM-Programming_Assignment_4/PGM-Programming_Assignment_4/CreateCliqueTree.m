...
%the C's factorlist is the initial input factorlist F 
C.factorList = F;
...
while cliquesConsidered < length(V)
	 ...
	 for i=1:size(edges,1)
		  score = sum(edges(i,:));
          %in the EliminateVar, once the a variable been eliminate its
          %edage link with other's variable will be 0, so the score=0,so
          %the following if statement doesn't consider the variable any
          %longer, and this guarantee that after length(V) loop all of the
          %variables will be traversed 
     ...
     %the new updated F is just used to update and create the clusters in
     %C(the C's factor list is the initial input F) 
     %note that even though a variable been eliminated, it still remain in the new
     %cluster,see the EliminateVar for detail
