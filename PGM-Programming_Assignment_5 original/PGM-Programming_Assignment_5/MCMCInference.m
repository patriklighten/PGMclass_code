...
if bSwendsenWang
    ...
    F_pairvar_indx=zeros(1,3);%F_pairvar_indx(i,1:2) is the variables index, F_pairvar_indx(i,3) is the index of the related entry in F 
            count=0;
            for i1=1:length(F)
                if (length(F(i1).var)==2)
                    if(count==0)
                        count=count+1;
                        F_pairvar_indx(1,1:2)=F(i1).var;
                        F_pairvar_indx(1,3)=i1;
                    else 
                        count=count+1;
                        temp_indx=[F(i1).var(1),F(i1).var(2),i1];
                        F_pairvar_indx=[F_pairvar_indx;temp_indx];
                    end 
                end
            end
  ...
  for i = 1:size([u, v], 1)
      ...
      if u(i) > v(i)
          ...
          elseif strcmp(TransName, 'MHSwendsenWang2')
              ...
              index=find(sum(ismember([u(i),v(i)],F_pairvar_indx(:,1:2)))==2);
              index=F_pairvar_indx(1,3);
              F_ij=F(index);
              
            %get the nominator of the q_ij
            assign1=ones(d,2).*[1:d;1:d]';
            nominator=GetValueOfAssignment(F_ij,assign1);
            nominator=sum( nominator);
            %get value of denominator of q_ij
            denominator=sum(F_ij.val);
            %compute q_ij     
            q_ij=nominator/denominator;
              %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
              E2F = EdgeToFactorCorrespondence(G.names, F);
