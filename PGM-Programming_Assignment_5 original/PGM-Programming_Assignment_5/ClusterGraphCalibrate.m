...
for m = 1:length(edgeFromIndx),
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    variables=intersect(P.clusterList(i).var,P.clusterList(j).var);
    card=zeros(1,length(variables));
    for i1=1:length(variables)
        card(i1)=P.clusterList(i).card(find(P.clusterList(i).var==variables(i1)));
    end
    MESSAGES(i,j).var=variables;
    MESSAGES(i,j).card=card;
    MESSAGES(i,j).val=ones(1,prod(card));
...
while (1),
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    edges=find(P.edges(:,i)==1);
    factor=P.clusterList(i);
    for i1=1:length(edges)
        if (edges(i1,1)~=j)
            factor=FactorProduct(factor,MESSAGES(edges(i1,1),i));
        end
        MESSAGES(i,j)=FactorMarginalization(factor,setdiff(factor.var,MESSAGES(i,j).var));
