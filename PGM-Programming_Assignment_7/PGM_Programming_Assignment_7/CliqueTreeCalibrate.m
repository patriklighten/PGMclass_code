...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    if (doLogZ)
        ...
        %find cardinal for every variables
        
        cardinalities = zeros(1, length(unique([P.cliqueList.var])));
        vars=1:length(unique([P.cliqueList.var]));
for i = 1:length(cardinalities)
    ...
    for j = 1:length(P.cliqueList)
        ...
        if ~isempty(find(P.cliqueList(j).var == i))
            ...
            % cardinality
            cardinalities(i) = P.cliqueList(j).card(find(P.cliqueList(j).var == i));
            break;
        end
    end
end
 ...
 P1=P;
    ...
    for i=1:N
        ...
        joint=FactorProduct(joint,unnormalizedMessages(i,j));
