...
%compute the reduce given the evidence 
Reduce=ObserveEvidence(Joint, E);
%normalize the Reduce 
Reduce.val=Reduce.val/sum(Reduce.val);
    
    
M = struct('var', [], 'card', [], 'val', []); % Returns empty factor. Change this.

remain_subset=setdiff(Joint.var,V);

M=FactorMarginalization(Reduce,remain_subset);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
