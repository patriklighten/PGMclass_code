...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            %vectirization method is more quickly, no need loop
            unconsist_indx=find(assignments(:,indx)~=x);
            F(j)=SetValueOfAssignment(F(j),assignments(unconsist_indx,:),0);
            
            %the following commented code is equivalent to above code, pthough need
            %loop and slowly , it is more understanderble 
            %for N=1:size(assignments,1)
          %      if  (assignments(N,indx)~=x)
          %          F(j)= SetValueOfAssignment(F(j),assignments(N,:),0);
          %      end 
           % end 
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
